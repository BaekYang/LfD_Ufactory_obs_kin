...
비디오 교시 : teaching\observational_teaching\
...
직접 교시 : teaching\kinesthetic_teaching\
...
★CoordinateTansform.py = 좌표계 변환(완성) 카메라→로봇암(x,y,z)
★ik_traj.py = x,y,z,r,p,y 값으로 joint1~joint6 계산 및 csv파일 출력

잘 움직이나 로봇암을 확인하는 코드 :
★run_scenario_result = 스페이스바 누르는 동안 csv파일 로봇암동작 재생(traj 팝업)
★remove_csv.py = 트레젝터리 튀는 값을 없애기 위해서 y값이 200보다 작으면 해당 행부터 마지막 행까지 삭제시키고 csv 출력
